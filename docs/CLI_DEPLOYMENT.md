...
> **Resource group naming:** All examples use `rg-mdo-attack-simulation`. Replace with your organization's naming convention. All scripts accept custom names via parameters.
...
To create the app registration (Step 2), the deploying user needs one of:

- **Cloud Application Administrator** - can create and manage app registrations
- **Application Administrator** - can create and manage all aspects of app registrations
- **Global Administrator** - full access (not recommended for routine use)

To grant admin consent for the Graph API permissions (`AttackSimulation.Read.All`, `User.Read.All`), the user also needs **Global Administrator** or **Privileged Role Administrator**.
