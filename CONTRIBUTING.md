...
This project follows a **trunk-based development** model with `main` as the single source of truth.

### Branch naming

| Pattern | Purpose | Lifetime |
|---|---|---|
| `main` | Stable, public-facing branch. Always deployable. | Permanent |
| `feat/<name>` | New features or enhancements | Short-lived (merge via PR, then delete) |
| `fix/<name>` | Bug fixes | Short-lived (merge via PR, then delete) |
| `docs/<name>` | Documentation-only changes | Short-lived (merge via PR, then delete) |
...
1. **Branch** from `main`:
   ...
   ```
2. **Commit** your changes following the commit conventions above
3. **Push** your branch and open a **Pull Request** targeting `main`
4. After review and CI checks pass, **merge** the PR
5. **Delete** the feature branch after merge

### Rules

- **Never push directly to `main`** - always use a Pull Request
- **Keep feature branches short-lived** - merge or close within a reasonable timeframe
- **Rebase or merge from `main`** regularly to stay current and avoid large merge conflicts
